...
<head>
<meta charset="utf-8">
<title>Articulos/Estudiantes</title>
...
<?php
	...
	$sql2="select codBlog,blog,fecha,categoria,cate,FKcreador,userAdmin from blog join cateblog on codCate=categoria join admin on codAdmin=FKcreador where categoria='6' order by fecha DESC";
	...
	$consulta="select codBlog,blog,fecha,categoria,cate,FKcreador,userAdmin from blog join cateblog on codCate=categoria join admin on codAdmin=FKcreador where categoria='6' order by fecha DESC LIMIT $empezar_desde,$tamano_paginas";
					...
					<?php 
						...
						{ 
							if($i != 1)
							{
								if($i < 8)
								{ ?>
								<li><a class="textts" href="?pagina=<?php echo $i ?>" style="font-size:15px"><?php echo $i ?></a></li><?php
								}
							}
					?>
					<?php } 
