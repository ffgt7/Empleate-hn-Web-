...
<?php
	...
	$sql=("update usuarios_empleo set nomb_user=?, nombres=?, apellidos=?, fk_nacionalida=?, sexo=?, fk_departamento=?, fk_municipio=?, direccion=?, 
	tel_fijo=?, tel_movil=?, Pos_vehi=?, Pos_moto=?, email_user=?, Fech_Naci=?, fk_TipoLicen=?, descrip_userEmpleo=?,
	identidadC=? where cod_empleo=?");

	$resultado=$conexion->prepare($sql);
	
	$resultado->execute(array($nomb_user,$nombres,$apellidos,$fk_nacionalida,$sexo,$fk_departamento,$fk_municipio,$direccion,$tel_fijo,$tel_movil,
	$Pos_vehi,$Pos_moto,$email_user,$fech_Naci,$tipLicen,$descrip_userEmpleo,$identidad,$cod)); 
